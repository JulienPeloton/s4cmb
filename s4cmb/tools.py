...
def get_healpix_ring_pixel_layout(nside, th_idx):
    ...
    if ith > nrings:
        startpix, nphi, kphi0, cth, sth = get_healpix_ring_pixel_layout(nside,ith  - 2 * (ith - nrings) -1)
        return 12 * nside ** 2 - startpix -nphi, nphi, kphi0, -cth, sth
    dth1 = 1. / 3. / nside ** 2
    dth2 = 2. / 3. / nside
    dst1 = 1. / (np.sqrt(6.) * nside)
    if ith < nside: # polar cap (north)
        cth = 1. - ith ** 2 * dth1
        nphi = 4 * ith
        kphi0 = 1
        sth = np.sin( 2.0 * np.arcsin(ith * dst1) ) # sin(theta)
        startpix = 2 * ith * (ith - 1)
    else:
        cth = (2 * nside - ith) * dth2
        nphi = 4 * nside
        kphi0 = (ith + 1 - nside) % 2
        sth = np.sqrt((1.0 -cth) * (1.0+cth)) #! sin(theta)
...
def alm2map_spin_der1(gclm, nside, spin, zbounds=(-1., 1.), ret_slice=None):
    """Returns spin-s transform '_{s}d' of alm, together with d/dtheta _{s}d and 1/sin tht d/dphi _{s}d.
    ...
    # shape (2, 12 * nside ** 2), first entry = real part, second entry imaginary part.
    _sd = np.array(hp.alm2map_spin(gclm, nside, spin, lmax))
    if spin > 1:
        _gclm = [hp.almxfl(gclm[0], get_alpha_lower(spin, lmax)), hp.almxfl(gclm[1], get_alpha_lower(spin, lmax))]
        _sm1d = np.array(hp.alm2map_spin(_gclm, nside, spin - 1, lmax))
    else:
        _sm1d = -np.array([hp.alm2map(hp.almxfl(gclm[0], get_alpha_lower(spin, lmax)), nside,verbose=False),
                           hp.alm2map(hp.almxfl(gclm[1], get_alpha_lower(spin, lmax)), nside,verbose=False)])

    _gclm = [hp.almxfl(gclm[0], get_alpha_raise(spin, lmax)), hp.almxfl(gclm[1], get_alpha_raise(spin, lmax))]
